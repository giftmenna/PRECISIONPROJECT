...
export default function CompetitionQuestionsPage() {
  ...
  const fetchQuestions = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/admin/questions?source=competition');
      ...
      try {
        ...
        console.error("JSON Parse Error. Raw response:", rawText, parseError);
        const parseErrorMessage = parseError instanceof Error ? parseError.message : String(parseError);
        throw new Error(`Failed to parse JSON response: ${parseErrorMessage}`);
      }
      ...
      const questionsWithChoices: Question[] = data.questions.map((q: any) => {
        let processedChoices: string[] = [];
        
        try {
          if (Array.isArray(q.choices)) {
            processedChoices = q.choices;
          } else if (typeof q.choices === 'string') {
            // Try to parse if it's a JSON string
            try {
              processedChoices = JSON.parse(q.choices);
            } catch {
              // If parsing fails, split by comma as fallback
              processedChoices = q.choices.split(',').map((c: string) => c.trim());
            }
          } else if (q.choices && typeof q.choices === 'object') {
            processedChoices = Object.values(q.choices);
          }
        } catch (choicesError) {
          console.error(`Error processing choices for question ${q.id}:`, choicesError);
        }

        return {
          ...q,
          choices: Array.isArray(processedChoices) ? processedChoices : []
        };
      });

      console.log("Processed questions:", questionsWithChoices);
      setQuestions(questionsWithChoices);
    } catch (error) {
      console.error("Error fetching questions:", error);
      alert(`Error fetching questions: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateQuestion = async () => {
    try {
      ...
      const apiData = {
        ...
        // Add the missing fields expected by the API
        isTestQuestion: true, 
        testCompetitionType: 'weekly', // Assuming 'weekly' as a default, adjust if needed
        notifyUsers: false // Add a default for notifyUsers
      };
  ...
  const filteredQuestions = questions.filter(question => {
    // Debug log for each question's choices
    console.log(`Question ${question.id} choices:`, question.choices);
    ...
    <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="w-full sm:w-auto">
            <h1 className="text-2xl sm:text-3xl font-bold mb-2">Weekly Competition Questions</h1>
            <div className="flex flex-wrap items-center gap-2 sm:gap-4">
              <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400">
                Manage questions for weekly competitions
              </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-md bg-white dark:bg-gray-800 dark:border-gray-600 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
            >
                ...
                <div className="flex flex-col sm:flex-row justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mb-3">
                      <h3 className="text-lg font-semibold line-clamp-2">{question.prompt}</h3>
                      <div className="flex flex-wrap gap-2">
                        <Badge variant="outline" className="whitespace-nowrap">{question.topic}</Badge>
                        <Badge 
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {(() => {
                        try {
                          const choicesArray = Array.isArray(question.choices) 
                            ? question.choices 
                            : (typeof question.choices === 'string' 
                              ? JSON.parse(question.choices) 
                              : []);
                          
                          return choicesArray.map((choice: string, index: number) => (
                            <div 
                              key={index} 
                              className={cn(
                            ...
                            >
                              ...
                              <span className={cn(
                                ...
                                choice === question.correct ? "font-semibold text-green-600" : ""
                              )}>
                                {choice}
                              </span>
                              {choice === question.correct && 
                          ...
                          console.error('Error processing choices for question:', question.id, error);
                          return null;
                        }
                      })()}
                    </div>
                  ...
                  <div className="flex gap-2 sm:ml-4 mt-4 sm:mt-0">
                    <Button 
                      ...
                      disabled={isEditing || isDeleting === question.id}
                      className="flex-1 sm:flex-none justify-center"
                    >
                      {isEditing ? (
                        <LoadingSpinner size="sm" className="mr-2" />
                    ...
                    <Button 
                      ...
                      disabled={isDeleting === question.id || isEditing}
                      className="flex-1 sm:flex-none justify-center"
                    >
